import { Plus, Filter, Edit3, Truck, Trash2 } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { vehicleAPI } from "../../api/vehicleAPI";
import { useSelector } from "react-redux";

const VehicleListView = ({ setIsListView, setEditId }) => {
  ...
  const getAllVehicle = async () => {
    try {
      ...
      const data = response?.paramObjectsMap?.vehicleVO?.data || [];
      const total = response?.paramObjectsMap?.vehicleVO?.totalCount || 0;
  ...
  return (
    <div className="max-w-7xl mx-auto mt-6 p-6 bg-white dark:bg-gray-900 rounded-xl shadow border border-gray-200 dark:border-gray-700">
            ...
            {loading ? (
              ...
              vehicles.map((v, idx) => (
                <tr key={v.id}
                  ...
                  <td className="px-5 py-3">{v.vehicleNumber}</td>
                  <td className="px-5 py-3">{v.vehicleType}</td>
      ...
      <div className="flex justify-between items-center mt-6 text-sm text-gray-600 dark:text-gray-300">
        ...
        <span>Showing {vehicles.length} of {totalCount} results</span>
